...
export default defineSource(async () => {
  const bases = [
    "https://czju.suzhou.gov.cn/czj/xwzx/list.shtml",
  ]
  ...
  function extractDateFromElement(a: cheerio.Cheerio<any>) {
    ...
    if (li.length) {
      // 优先查找常见的日期元素
      candidates.push(li.find("span.youce").first().text())  // 苏州财政特有的日期元素
    ...
    candidates.push(a.parent().find("time, .date, .time, span.youce").first().text())
    ...
    try {
      ...
      const items = $links
        .map((_, el) => {
          ...
          const u = new URL(url)
          // 仅限新闻中心栏目详情页：路径含 /czj/xwzx/ 且是内容页 .shtml，排除列表/索引页
          const inNewsCenter = u.pathname.includes("/czj/xwzx/")
          const isContentPage = u.pathname.endsWith(".shtml") && !u.pathname.endsWith("list.shtml") && !u.pathname.endsWith("index.shtml")
          if (!inNewsCenter || !isContentPage) return null
  ...
  // 补采集：对未解析到时间的条目抓取详情页发布时间（限制并发与数量，避免过载）
  const needDetail = unique.filter(i => !i.pubDate)
  const batch = needDetail.slice(0, 30) // 最多补 30 条
