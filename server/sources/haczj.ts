...
export default defineSource(async () => {
  const bases = [
    "http://czj.huaian.gov.cn/xwdt/gzdt/list.html",
  ]
  ...
  function extractDateFromElement(a: cheerio.Cheerio<any>) {
    ...
    if (li.length) {
      candidates.push(li.find(".lb-time, .time").first().text())
      candidates.push(li.find("span.lb-time").first().text())
  ...
  function extractDateFromUrl(url: string) {
    try {
      ...
      // /xwdt/gzdt/content/17592480/1761895335498eoWB6QKq.html
      // 无法从URL中提取日期
      return undefined
    } catch {}
    return undefined
    ...
    try {
      ...
      const items = $links
        .map((_, el) => {
          ...
          const u = new URL(url)
          if (u.hostname !== "czj.huaian.gov.cn") return null
          
          // 只抓取 /xwdt/gzdt/content/ 下的详情页
          const isContentPage = /\/xwdt\/gzdt\/content\//.test(u.pathname) && u.pathname.endsWith(".html")
