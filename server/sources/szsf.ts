...
export default defineSource(async () => {
  const bases = [
    "https://www.suzhou.gov.cn/szsrmzf/szyw/nav_list.shtml",
  ]
  ...
  function extractDateFromElement(a: cheerio.Cheerio<any>) {
    ...
    for (const raw of candidates) {
      ...
      // 如果标准格式不匹配，尝试其他格式
      // 但要注意：旧正则可能会匹配到部分日期（如"2025-12-3"），所以要验证日期是否完整
      m = text.match(datePattern)
      if (m) {
        const matched = m[0]
        // 对于标准格式的日期（包含-分隔符），验证日期部分是否完整（至少2位）
        if (matched.includes('-')) {
          const parts = matched.split(/[-.\/年月]/)
          if (parts.length >= 3 && parts[2] && parts[2].trim().length >= 1) {
            // 日期部分存在，使用normalizeDateString处理
            const normalized = normalizeDateString(matched)
            const ts = Date.parse(normalized)
  ...
  function extractDateFromUrl(url: string) {
    try {
      ...
      // /szsrmzf/szyw/202511/60c7fd4962d246eaa03116534401677c.shtml
      // URL中只有年月，无法提取精确日期
    ...
    try {
      ...
      const items = $links
        .map((_, el) => {
          ...
          // 排除导航/栏目标题链接，包括"苏州要闻"
          if (/^(网站首页|咨询中心|首页|联系我们|政务服务|互动交流|专题|专题专栏|政声传递|站点地图|网站地图|公告公示|隐私安全|苏州要闻)$/i.test(title.trim())) return null
          title = title.replace(/\s+/g, " ")

          const url = href.startsWith("http") ? href : new URL(href, baseUrl).toString()
          const u = new URL(url)
          // 确保是 suzhou.gov.cn 域名
          if (u.hostname !== "www.suzhou.gov.cn") return null
          // 排除列表页链接（nav_list.shtml 等）
          if (/nav_list\.shtml|index(_\d+)?\.(html|shtml)$/.test(u.pathname)) return null
          // 确保是详情页，不是列表页，并且是 szsrmzf/szyw 路径
          const isContentPage = (u.pathname.endsWith(".html") || u.pathname.endsWith(".shtml")) && !/index(_\d+)?\.(html|shtml)$/.test(u.pathname) && u.pathname.includes("/szsrmzf/szyw/")
  ...
  async function fetchDetailPubDate(url: string) {
    try {
      ...
      for (const raw of textCandidates) {
        ...
        let m = searchText.match(standardPattern)
        if (m) {
          const [, year, month, day] = m
          const normalized = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`
          const ts = Date.parse(normalized)
  ...
  // 过滤掉"苏州要闻"栏目标题（使用包含匹配，更严格）
  const filtered = unique.filter(item => {
    const t = item.title.trim()
    return t !== "苏州要闻" && !t.includes("苏州要闻") && !/^苏州要闻[\s\-_]/i.test(t)
  })
  return filtered.sort((a, b) => (b.pubDate || 0) - (a.pubDate || 0))
})
