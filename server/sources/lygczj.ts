...
export default defineSource(async () => {
  const bases = [
    "http://czj.lyg.gov.cn/cjxx/cjxx.html",
  ]
  ...
  function extractDateFromUrl(url: string) {
    try {
      ...
      // /lygsczj/cjxw/content/xxx.html 或 /lygsczj/tzgg/content/xxx.html
  ...
  async function fetchOne(listUrl: string) {
    try {
      ...
      if (!initData.length) {
        ...
        const items = $links
          .map((_, el) => {
            ...
            const u = new URL(url)
            if (u.hostname !== "czj.lyg.gov.cn") return null
      ...
      // 从initData中提取
      const $links = initData.find("ul li a")
      const baseUrl = new URL(listUrl)
