...
export default defineSource(async () => {
  const bases = [
    "https://czj.changzhou.gov.cn/class/OJPCNLOE",
  ]
  ...
  function extractDateFromUrl(url: string) {
    try {
      ...
      // /html/czfb/2025/OJPCNLOE_1103/xxx.html
      const m1 = path.match(/\/html\/\w+\/(20\d{2})\/\w+_(\d+)\//)
      if (m1) {
        const [_, y, mmdd] = m1
        const mo = mmdd.substring(0, 2)
        const d = mmdd.substring(2, 4)
    ...
    try {
      ...
      const items = $links
        .map((_, el) => {
          ...
          const u = new URL(url)
          if (u.hostname !== "czj.changzhou.gov.cn") return null
          const isContentPage = (u.pathname.endsWith(".html") || u.pathname.endsWith(".shtml")) && !/index(_\d+)?\.html$/.test(u.pathname) && !/class\/\w+/.test(u.pathname)
