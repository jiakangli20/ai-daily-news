...
export default defineSource(async () => {
  const bases = [
    "https://sqcz.suqian.gov.cn/sczj/gzdt/list.shtml",
  ]
  ...
  function extractDateFromUrl(url: string) {
    try {
      ...
      // /sczj/gzdt/202510/de2ba869f9ed40799590ad03d2c6544c.shtml
    ...
    try {
      ...
      const items = $links
        .map((_, el) => {
          ...
          const u = new URL(url)
          if (u.hostname !== "sqcz.suqian.gov.cn") return null
          
          // 只抓取 /sczj/gzdt/ 下的详情页
          const isContentPage = (u.pathname.endsWith(".html") || u.pathname.endsWith(".shtml")) && !/index(_\d+)?\.(html|shtml)$/.test(u.pathname) && /\/sczj\/gzdt\/\d+\//.test(u.pathname)
